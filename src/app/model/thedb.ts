...
export class TheDb {
    ...
    public static createDb(dbPath: string): Promise<string> {
        ...
        const dataPath = path.join(Settings.dbFolder, `qlchvangbac.db`);
        const schemaPath = path.join(Settings.dbFolder, `qlchvangbac.db.sql`);
        sqlite.decrypt(dataPath, schemaPath, "admin", "aes-256-ctr");
        console.log('Test: ', schemaPath);
